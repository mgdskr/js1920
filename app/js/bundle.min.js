...
(function () {
    ...
    $(function () {
        ...
        $('.jcarousel-pagination').on('jcarouselpagination:active', 'input', function () {
...
//chaining
var skills = _.chain(data).map('skills').flattenDeep().uniq().sortBy().value();
...
// --------------2---------------
var sortedByFriendsNum = _.chain(data).sortBy('friends.length').map('name').value();

console.log('2 Names sorted by friends number (ASC)', sortedByFriendsNum);

// --------------3---------------
var friends = _.chain(data).map('friends').flattenDeep().map('name').uniq().value();
